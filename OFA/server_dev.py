...
garbage_classification = pipeline(
    ...
    device="gpu:1")
ts2 = time.time()
print("【加载定位、图像分类、垃圾分类模型用时】: ", ts2-ts1)
...
def is_vg_reliable(imgclass_result, text):
    ...
    img_labels = imgclass_result["labels"][0:1]
...
def garbage_cls():
    ...
    # 执行垃圾分类模型
    result = garbage_classification(img_file_path)
    ...
    result_filepath = img_file_path.replace(".jpg", ".json")
